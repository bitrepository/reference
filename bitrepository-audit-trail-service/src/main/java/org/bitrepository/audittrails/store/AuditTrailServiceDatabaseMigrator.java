...
public class AuditTrailServiceDatabaseMigrator extends DatabaseMigrator {
    ...
    public void migrate() {
        ...
        if (!versions.containsKey(AUDIT_TRAIL_TABLE)) {
            throw new IllegalStateException("The database does not contain '" + AUDIT_TRAIL_TABLE
                    + "' table as required.");
        }
        ...
        if (!versions.containsKey(DATABASE_VERSION_ENTRY) || versions.get(DATABASE_VERSION_ENTRY) < 6) {
