...
public class AuditServiceDatabaseMigrationTest extends ExtendedTestCase {
    ...
    static final String PATH_TO_DATABASE_JAR_FILE = new File(
            Objects.requireNonNull(Thread.currentThread().getContextClassLoader().getResource("auditservicedb-v2.jar"))
    ...
    public void setup() throws Exception {
        ...
        settings.getReferenceSettings().getAuditTrailServiceSettings().getAuditTrailServiceDatabase()
                .setDatabaseURL("jdbc:derby:" + PATH_TO_DATABASE_UNPACKED + "/auditservicedb");

        DatabaseSpecifics auditDB = settings.getReferenceSettings().getAuditTrailServiceSettings().getAuditTrailServiceDatabase();
