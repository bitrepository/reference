...
public abstract class ReplaceFileSelector extends ComponentSelector {
     ...
     */
    abstract protected boolean checkPillarResponseForSelection(IdentifyPillarsForReplaceFileResponse response) 
     ...
     * expected pillar.
     *
     * Consider overriding this in subclasses to perform type checking og the message before delegating back to this
    ...
    public void processResponse(MessageResponse response) throws UnexpectedResponseException {
        if (response instanceof IdentifyPillarsForReplaceFileResponse) {
            IdentifyPillarsForReplaceFileResponse resp = (IdentifyPillarsForReplaceFileResponse) response;
