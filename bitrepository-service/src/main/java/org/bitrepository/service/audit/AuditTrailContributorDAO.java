 ...
 */
public abstract class AuditTrailContributorDAO implements AuditTrailManager {
     ...
     */
    public AuditTrailContributorDAO(DatabaseManager manager) {
        this.dbConnector = manager.getConnector();
        getConnection();
    }

    /**
     * Initialization method, needed to set up the instance.
     *
     * @param componentID The ID of the component using the AuditTrailContributorDAO instance
     */
    ...
    protected void addAuditEvent(String collectionID, String fileID, Date auditTime, String actor, String info,
        ...
        synchronized (this) {
            String insertSql = "INSERT INTO " + AUDIT_TRAIL_TABLE + " ( " + AUDIT_TRAIL_FILE_GUID + " , "
                    + AUDIT_TRAIL_ACTOR_GUID + " , " + AUDIT_TRAIL_OPERATION + " , " + AUDIT_TRAIL_OPERATION_DATE + " , "
    ...
    public Long extractLargestSequenceNumber() {
        String sql = "SELECT " + AUDIT_TRAIL_SEQUENCE_NUMBER + " FROM " + AUDIT_TRAIL_TABLE + " ORDER BY "
                + AUDIT_TRAIL_SEQUENCE_NUMBER + " DESC";
    ...
    private AuditTrailDatabaseResults extractEvents(AuditTrailExtractor extractor) {
        ...
        String sql = "SELECT " + AUDIT_TRAIL_SEQUENCE_NUMBER + ", "
                + FILE_FILE_ID + " , "
                + AUDIT_TRAIL_ACTOR_GUID + " , "
                + AUDIT_TRAIL_OPERATION_DATE + " , "
                + AUDIT_TRAIL_OPERATION + " , "
                + AUDIT_TRAIL_AUDIT + " , "
                + AUDIT_TRAIL_INFORMATION + " , "
                + AUDIT_TRAIL_OPERATIONID + " , "
                + AUDIT_TRAIL_FINGERPRINT
                + " FROM "
                + AUDIT_TRAIL_TABLE + " JOIN " + FILE_TABLE
    ...
    private synchronized long retrieveFileGuid(String collectionID, String fileID) {
        String sqlRetrieve = "SELECT " + FILE_GUID + " FROM " + FILE_TABLE + " WHERE " + FILE_FILE_ID + " = ? AND "
                + FILE_COLLECTION_ID + " = ?";
    ...
    /**
     * Private class for encapsulating the request for extracting.
     */
    private class AuditTrailExtractor {
        ...
        public String createRestriction() {
            ...
            if (collectionID != null) {
                nextArgument(res);
                res.append(FILE_COLLECTION_ID + " = ?");
            }

            if (fileID != null) {
                nextArgument(res);
                res.append(FILE_FILE_ID + " = ?");
            }

            if (minSeqNumber != null) {
                nextArgument(res);
                res.append(AUDIT_TRAIL_SEQUENCE_NUMBER + " >= ?");
            }
            ...
            if (minDate != null) {
                nextArgument(res);
                res.append(AUDIT_TRAIL_OPERATION_DATE + " >= ?");
            }
            ...
            res.append(" ORDER BY " + AUDIT_TRAIL_SEQUENCE_NUMBER);
