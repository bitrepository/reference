...
import org.bitrepository.integrityservice.IntegrityDatabaseTestCaseFileInfos;
 ...
 */
public class HandleChecksumValidationStepTestFileInfos extends IntegrityDatabaseTestCaseFileInfos {
    ...
    public void testChecksumMajority() throws Exception {
        addDescription(
                "Test the checksum integrity validator when two pillars have one checksum and the last pillar " + "has another checksum.");
        IntegrityModel cache = getIntegrityModel();
        IntegrityReporter reporter = new BasicIntegrityReporter(TEST_COLLECTION, "test", new File("target/"));
        StatisticsCollector cs = new StatisticsCollector(TEST_COLLECTION);
        HandleChecksumValidationStep step = new HandleChecksumValidationStep(cache, auditManager, reporter, cs);
    ...
    private class TestAuditTrailManager implements AuditTrailManager {
        ...
        public void addAuditEvent(String collectionID, String fileID, String actor, String info, String auditTrail, FileAction operation,
                                  String operationID, String certificateID) {
