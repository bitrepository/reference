...
public class IntegrityDatabaseTestFileInfos extends IntegrityDatabaseTestCase {
    ...
    protected void customizeSettings() {
        org.bitrepository.settings.repositorysettings.Collection c0 = settings.getRepositorySettings().getCollections().getCollection()
                .get(0);
    ...
    public void initialStateExtractionTest() {
        ...
        Collection<String> oldChecksums = getIssuesFromIterator(
                model.findChecksumsOlderThan(new Date(0), TEST_PILLAR_1, TEST_COLLECTIONID));
        ...
        for (String pillar : SettingsUtils.getPillarIDsForCollection(TEST_COLLECTIONID)) {
            Collection<String> missingChecksums = getIssuesFromIterator(
                    model.findFilesWithMissingChecksum(TEST_COLLECTIONID, pillar, new Date(0)));
        ...
        Collection<String> missingFiles = getIssuesFromIterator(
                model.findFilesWithMissingCopies(TEST_COLLECTIONID, SettingsUtils.getPillarIDsForCollection(TEST_COLLECTIONID).size(), 0L,
                        Long.MAX_VALUE));
