 ...
 */
public class SimpleFileInfosEventHandler implements EventHandler {
    ...
    private final Map<String, ResultingFileInfos> fileInfoResults = new HashMap<>();
     ...
     */
    public SimpleFileInfosEventHandler(long timeout, IntegrityContributors integrityContributors) {
    ...
    private void handleResult(OperationEvent event) {
        if (event instanceof FileInfosCompletePillarEvent) {
            ...
            log.trace("Receiving GetChecksums result: {}", fileInfoEvent.getFileInfos().getFileInfosDataItem().toString());
