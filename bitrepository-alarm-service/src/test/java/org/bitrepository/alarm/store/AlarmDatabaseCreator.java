...
public class AlarmDatabaseCreator extends DatabaseCreator {
    ...
    public static void main(String[] args) {
        AlarmDatabaseCreator dbCreator = new AlarmDatabaseCreator();
        Settings settings = dbCreator.loadSettings(null, args[0]);

        dbCreator.createAlarmDatabase(settings, args[1]);
    }

    public void createAlarmDatabase(Settings settings, String pathToSqlCreationScript) {
        ...
        if (pathToSqlCreationScript == null) {
            pathToSqlCreationScript = DEFAULT_ALARM_DB_SCRIPT;
        }
