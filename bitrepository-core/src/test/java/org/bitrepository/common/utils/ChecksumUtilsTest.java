...
public class ChecksumUtilsTest extends ExtendedTestCase {
    ...
    public void calculateHmacChecksums() throws Exception {
        ...
        Assert.assertEquals(ChecksumUtils.generateChecksum(data1, csHmacMD5), "74e6f7298a9c2d168935f58c001bad88");
        Assert.assertEquals(ChecksumUtils.generateChecksum(data1, csHmacSHA1), "fbdb1d1b18aa6c08324b7d64b71fb76370690e1d");
        ...
        addStep("Test with the text '" + message + "' and key '" + key + "' for MD5, SHA1, and SHA256", "Should give expected results.");
        Assert.assertEquals(ChecksumUtils.generateChecksum(data2, csHmacMD5), "80070713463e7749b90c2dc24911e275");
        data2.reset();
        Assert.assertEquals(ChecksumUtils.generateChecksum(data2, csHmacSHA1), "de7c9b85b8b78aa6bc8a7a36f70a90701c9db4d9");
        data2.reset();
    ...
    public void calculateDigestChecksums() throws Exception {
        ...
        InputStream data1 = new ByteArrayInputStream(new byte[0]);
        Assert.assertEquals(ChecksumUtils.generateChecksum(data1, csMD5), "d41d8cd98f00b204e9800998ecf8427e");
        Assert.assertEquals(ChecksumUtils.generateChecksum(data1, csSHA1), "da39a3ee5e6b4b0d3255bfef95601890afd80709");
        ...
        String message = "The quick brown fox jumps over the lazy dog";
        InputStream data2 = new ByteArrayInputStream(message.getBytes(StandardCharsets.UTF_8));
        Assert.assertEquals(ChecksumUtils.generateChecksum(data2, csMD5), "9e107d9d372bb6826bd81d3542a419d6");
        data2.reset();
        Assert.assertEquals(ChecksumUtils.generateChecksum(data2, csSHA1), "2fd4e1c67a2d28fced849ee1bb76e7391b93eb12");
        data2.reset();
    ...
    public void testChecksumOnFile() throws Exception {
        ...
        File testFile = new File(
                Objects.requireNonNull(Thread.currentThread().getContextClassLoader().getResource("test-files/default-test-file.txt"))
                        .getFile());
    ...
    private void validateHmac(ChecksumType hmacType) throws NoSuchAlgorithmException {
        addStep("Test '" + hmacType + "'", "Should be invalid without salt, and valid with no matter whether " + "the salt is empty.");
        ChecksumSpecTYPE csType = new ChecksumSpecTYPE();
    ...
    private void validateMessageDigest(ChecksumType algorithmType) throws NoSuchAlgorithmException {
        addStep("Test '" + algorithmType + "'",
                "Should be valid without salt, valid with an empty salt, " + "and invalid with a proper salt.");
        ChecksumSpecTYPE csType = new ChecksumSpecTYPE();
