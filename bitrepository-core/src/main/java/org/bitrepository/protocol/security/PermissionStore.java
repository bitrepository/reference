...
public class PermissionStore {
     ...
     */
    private final Map<CertificateID, CertificatePermission> permissionMap;
     ...
     */
    public void loadPermissions(PermissionSet permissions, String componentID) {
        if (permissions != null) {
            ...
            for (Permission permission : permissions.getPermission()) {
                try {
                    if (permission.getCertificate().getAllowedCertificateUsers() != null) {
                        allowedUsers = new HashSet<>(permission.getCertificate().getAllowedCertificateUsers().getIDs());
                    } else {
                        allowedUsers = null;
                    }
                    ...
                    if (permission.getOperationPermission() != null) {
                        for (OperationPermission perm : permission.getOperationPermission()) {
                            if (perm.getAllowedComponents() == null || perm.getAllowedComponents().getIDs().contains(componentID)) {
                    ...
                    if (certificate != null) {
                        ...
                        CertificatePermission certificatePermission = new CertificatePermission(certificate, allowedOperationPermissions,
                                allowedUsers);
    ...
    public X509Certificate getCertificate(SignerId signer) throws PermissionStoreException {
        ...
        if (permission != null) {
            ...
            throw new PermissionStoreException("Failed to find certificate for the requested signer:" + certificateID);
        }
    }
    ...
    private X509Certificate makeCertificate(byte[] certificateData) throws CertificateException {
        ...
        X509Certificate certificate = (X509Certificate) CertificateFactory.getInstance(SecurityModuleConstants.CertificateType)
                .generateCertificate(bs);
        try {
            bs.close();
        } catch (IOException e) {
     ...
     */
    private static final class CertificatePermission {
        /**
        ...
        /**
         * @param certificate                 the certificate which permissions are to be represented.
         * @param allowedOperationPermissions a set of allowed operation permissions.
