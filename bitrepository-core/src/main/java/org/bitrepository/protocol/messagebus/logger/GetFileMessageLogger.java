...
public class GetFileMessageLogger extends DefaultMessagingLogger {
    ...
    protected void appendCustomInfo(StringBuilder messageSB, Message message) {
        if (message instanceof IdentifyPillarsForGetFileRequest) {
            ...
            messageSB.append(" FileID=").append(getFileRequest.getFileID());
            if (getFileRequest.getFilePart() != null) {
                messageSB.append(", FilePart=").append(getFileRequest.getFilePart());
            }
            messageSB.append(", FileAddress=").append(getFileRequest.getFileAddress());
        }
    }
}
