...
public class GetFileInfosEventHandler extends PagingEventHandler {
    private final GetFileInfosResultModel model;

    public GetFileInfosEventHandler(GetFileInfosResultModel model, Long timeout, OutputHandler outputHandler) {
    ...
    protected void handleResult(OperationEvent event) {
        if (event instanceof FileInfosCompletePillarEvent) {
            FileInfosCompletePillarEvent pillarEvent = (FileInfosCompletePillarEvent) event;
            if (pillarEvent.isPartialResult()) {
                pillarsWithPartialResults.add(pillarEvent.getContributorID());
            }
            model.addResults(pillarEvent.getContributorID(), pillarEvent.getFileInfos());
        }
    }
}