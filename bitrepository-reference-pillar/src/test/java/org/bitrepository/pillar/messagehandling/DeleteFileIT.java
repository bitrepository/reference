...
import org.bitrepository.pillar.MockedPillarIT;
...
class DeleteFileIT extends MockedPillarIT {
    ...
    void goodCaseIdentification() throws Exception {
        ...
        Mockito.doAnswer(invocation -> true).when(model).hasFileID(eq(FILE_ID), anyString());
    ...
    void badCaseIdentification() throws Exception {
        ...
        Mockito.doAnswer(invocation -> false).when(model).hasFileID(eq(FILE_ID), anyString());
        Mockito.doAnswer(invocation -> settingsForCUT.getComponentID()).when(model).getPillarID();
    ...
    @SuppressWarnings("rawtypes")
    //@Test
//    @Tag(TestGroups.REGRESSIONTEST, "pillartest"})
