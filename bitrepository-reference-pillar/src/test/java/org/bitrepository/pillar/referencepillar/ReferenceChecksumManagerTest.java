...
public class ReferenceChecksumManagerTest extends DefaultFixturePillarTest {
    ...
    private void testRecalculatingChecksumsDueToChangedFile(ReferenceChecksumManager csManager, CollectionArchiveManager archives) throws Exception {
        ...
        ExtractedChecksumResultSet initialCsResults = csManager.getEntries(null, null, null, settingsForCUT.getCollectionID());
        ...
        archives.downloadFileForValidation(TEST_FILE, settingsForCUT.getCollectionID(), new ByteArrayInputStream(TEST_CONTENT.getBytes()));
        archives.moveToArchive(TEST_FILE, settingsForCUT.getCollectionID());
        ExtractedChecksumResultSet csResults = csManager.getEntries(null, null, null, settingsForCUT.getCollectionID());
        ...
        Assert.assertTrue(archives.getFile(TEST_FILE, settingsForCUT.getCollectionID()).lastModified() >= initialDate.getTime()); 
        Assert.assertTrue(archives.getFile(TEST_FILE, settingsForCUT.getCollectionID()).lastModified() <= beforeChange.getTime());
        ...
        FileOutputStream out = new FileOutputStream(archives.getFile(TEST_FILE, settingsForCUT.getCollectionID()), true);
        out.write("\n".getBytes());
        out.write(TEST_ADDED_CONTENT.getBytes());
        out.flush();
        out.close();

        ExtractedChecksumResultSet changedCsResults = csManager.getEntries(null, null, null, settingsForCUT.getCollectionID());
        ...
        Assert.assertTrue(archives.getFile(TEST_FILE, settingsForCUT.getCollectionID()).lastModified() >= initialDate.getTime());
        Assert.assertTrue(archives.getFile(TEST_FILE, settingsForCUT.getCollectionID()).lastModified() >= beforeChange.getTime());
        Assert.assertTrue(archives.getFile(TEST_FILE, settingsForCUT.getCollectionID()).lastModified() <= afterChange.getTime());
