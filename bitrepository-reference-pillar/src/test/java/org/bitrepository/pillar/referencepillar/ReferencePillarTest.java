...
public abstract class ReferencePillarTest extends DefaultFixturePillarTest {
    ...
    private void initializeArchiveWithEmptyFile() {
        ...
        try {
            archives.downloadFileForValidation(DEFAULT_FILE_ID, settingsForCUT.getCollectionID(), new ByteArrayInputStream(new byte[0]));
            archives.moveToArchive(DEFAULT_FILE_ID, settingsForCUT.getCollectionID());
            csCache.insertChecksumCalculation(DEFAULT_FILE_ID, settingsForCUT.getCollectionID(), EMPTY_FILE_CHECKSUM, new Date());
        } catch (Exception e) {
