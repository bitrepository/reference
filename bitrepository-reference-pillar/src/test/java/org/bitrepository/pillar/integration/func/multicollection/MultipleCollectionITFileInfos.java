...
public class MultipleCollectionITFileInfos extends PillarIntegrationTest {
    /**
    ...
    public void fileInOtherCollectionTest() throws Exception {
        ...
        clientProvider.getPutClient()
                .putFile(nonDefaultCollectionId, DEFAULT_FILE_URL, NON_DEFAULT_FILE_ID, 10L, TestFileHelper.getDefaultFileChecksum(), null,
                        null, null);
        ...
        Assert.assertEquals(1, clientProvider.getGetFileInfosClient()
                .getFileInfos(nonDefaultCollectionId, new ContributorQuery[]{query}, NON_DEFAULT_FILE_ID, checksumspec, DEFAULT_FILE_URL,
                        null, null).size());

        addStep("Send a getFileIDs for the file in the other collections", "The file should not be found here");
        try {
            int size = clientProvider.getGetFileInfosClient()
                    .getFileInfos(collectionID, new ContributorQuery[]{query}, NON_DEFAULT_FILE_ID, checksumspec, DEFAULT_FILE_URL, null,
                            null).size();
    ...
    protected void registerMessageReceivers() {
        ...
        Collection<String> pillarFilter = List.of(testConfiguration.getPillarUnderTestID());
