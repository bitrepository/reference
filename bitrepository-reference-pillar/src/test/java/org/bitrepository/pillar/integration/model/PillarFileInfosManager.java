...
public class PillarFileInfosManager {
    ...
    public PillarFileInfosManager(String collectionID, String pillarID, Settings mySettings, ClientProvider clientProvider,
                                  TestEventManager testEventManager, HttpServerConfiguration httpServerConfiguration) {
    ...
    public void deleteAllFiles() {
        ...
        for (FileInfosDataItem checksumData : filesWithChecksums) {
            ...
            try {
                clientProvider.getDeleteFileClient()
                        .deleteFile(collectionID, checksumData.getFileID(), pillarID, checksumDataForFile, null, null, "");
     ...
     * how many files where found last time, based on the assumption that the number of files between calls will
     * now decrease. If to few files are initially present, the remaining files are put to the pillar.
     *
     * @param desiredNumberOfFiles The desired number of files
     */
    ...
    public void addFilesToPillar(String collectionID, int numberOfFilesToAdd, String testName) {
        ...
        for (String newFileID : newFileIDs) {
            try {
                ...
                clientProvider.getPutClient()
                        .putFile(collectionID, httpServerConfiguration.getURL(TestFileHelper.DEFAULT_FILE_ID), newFileID, 10L,
    ...
    public List<FileInfosDataItem> getFileInfos(ChecksumSpecTYPE checksumSpec, ContributorQuery query, String fileID) {
        ...
        try {
            List<ContributorEvent> result = clientProvider.getGetFileInfosClient()
                    .getFileInfos(collectionID, new ContributorQuery[]{query}, fileID, checksumSpec, null, null, null);
