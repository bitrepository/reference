...
public class PillarPerformanceIT extends PillarIntegrationIT {
    ...
    protected void awaitAsynchronousCompletion(Metrics metrics, int numberOfOperations) {
        while(metrics.getCount() < numberOfOperations) {
            log.info("...waiting for the last {} operations to finish ({})", (numberOfOperations - metrics.getCount()), TimeUtils.millisecondsToHuman(System.currentTimeMillis()-metrics.getStartTime()));
            try {
                Thread.sleep(6000);
        ...
        log.info("All operations finished in {}",TimeUtils.millisecondsToHuman(System.currentTimeMillis()-metrics.getStartTime()));
    }

    protected static class EventHandlerForMetrics implements EventHandler {
    ...
    protected static class ParallelOperationLimiter {
