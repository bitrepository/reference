...
public class ChecksumDatabaseMigrationTest extends ExtendedTestCase {
    ...
    static final String PATH_TO_DATABASE_V1_JAR_FILE = new File(
            Objects.requireNonNull(Thread.currentThread().getContextClassLoader().getResource("checksumdb-version1.jar"))
                    .getFile()).getAbsolutePath();
    static final String PATH_TO_DATABASE_V3_JAR_FILE = new File(
            Objects.requireNonNull(Thread.currentThread().getContextClassLoader().getResource("checksumdb" + "-version3.jar"))
                    .getFile()).getAbsolutePath();
    ...
    public void setup() throws Exception {
        ...
        settings.getReferenceSettings().getPillarSettings().getChecksumDatabase()
                .setDatabaseURL("jdbc:derby:" + PATH_TO_DATABASE_UNPACKED + "/checksumdb");
    ...
    public void testMigratingChecksumDatabaseFromV1ToV2() throws Exception {
        ...
        connector = new DBConnector(settings.getReferenceSettings().getPillarSettings().getChecksumDatabase());
        ...
        String insertSql =
                "INSERT INTO " + CHECKSUM_TABLE + " ( " + CS_FILE_ID + " , " + CS_CHECKSUM + " , " + CS_DATE + " ) VALUES ( ? , ? , ? )";
        ...
        String retrieveCollectionIdSql = "SELECT " + CS_COLLECTION_ID + " FROM " + CHECKSUM_TABLE + " WHERE " + CS_FILE_ID + " = ?";
    ...
    public void testMigratingChecksumDatabaseFromV3ToV4() throws Exception {
        ...
        String insertSql =
                "INSERT INTO " + CHECKSUM_TABLE + " ( " + CS_FILE_ID + " , " + CS_CHECKSUM + " , " + CS_DATE + " , " + CS_COLLECTION_ID +
                        " ) VALUES ( ? , ? , ? , ? )";
        ...
        String retrieveCollectionIdSql = "SELECT " + CS_DATE + " FROM " + CHECKSUM_TABLE + " WHERE " + CS_FILE_ID + " = ?";
        Long extractedDate = DatabaseUtils.selectFirstLongValue(connector, retrieveCollectionIdSql, FILE_ID);

        Date testDateAtTimeZone = new Date(
                testDate.getTime() + Calendar.getInstance(TimeZone.getDefault(), Locale.ROOT).getTimeZone().getRawOffset());
