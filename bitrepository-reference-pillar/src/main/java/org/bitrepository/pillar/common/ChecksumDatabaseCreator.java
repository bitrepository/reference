...
/**
 * Used to create a checksum database.<br>
 * Defines the concrete DatabaseSpecifics and SQL-script location.
 *
 * @see DatabaseCreator DatabaseCreater
 */
public class ChecksumDatabaseCreator extends DatabaseCreator {
    ...
    public void createChecksumDatabase(Settings settings, String pathToSqlCreationScript) {
        DatabaseSpecifics databaseSpecifics = settings.getReferenceSettings().getPillarSettings().getChecksumDatabase();

        createDatabase(databaseSpecifics, pathToSqlCreationScript == null ? DEFAULT_CHECKSUM_DB_SCRIPT : pathToSqlCreationScript);
    }
}
