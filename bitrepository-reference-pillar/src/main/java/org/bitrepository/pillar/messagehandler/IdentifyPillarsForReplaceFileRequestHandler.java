 ...
 */
public class IdentifyPillarsForReplaceFileRequestHandler extends IdentifyRequestHandler<IdentifyPillarsForReplaceFileRequest> {
     ...
     * @throws IdentifyContributorException If the file is not available, e.g. it doesn't exist.
     */
    private void checkThatRequestedFileIsAvailable(IdentifyPillarsForReplaceFileRequest message) throws IdentifyContributorException {
        if (!getPillarModel().hasFileID(message.getFileID(), message.getCollectionID())) {
            throw new IdentifyContributorException(ResponseCode.FILE_NOT_FOUND_FAILURE,
                    "Could not find the " + "requested file to delete.");
        }
    }
